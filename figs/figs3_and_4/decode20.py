...
stim = np.load('rawData/stim20Hz.npy')
...
for pi in range(numPcon):
    ...
    for seed in range(numseeds):
        print(f'\tseed {seed}')
        if not os.path.exists(f'decodeData/seed{seed}'):
            os.mkdir(f'decodeData/seed{seed}')
        ...
        for dti in range(numDeltas):
            ...
            for li in range(numLayers):
                ...
                loadpath = f'rawData/seed{seed}/{layer}_spks.npy'
                spk = np.load(loadpath)
            
                savepath = f'decodeData/seed{seed}/deltat{deltat}'
