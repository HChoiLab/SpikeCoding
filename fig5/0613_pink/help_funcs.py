...
class stim_freq(torch.utils.data.Dataset):
    ...
    def __init__(self, timesteps, num_samples, f):
        ...
        c = 0.5*torch.cos(2.*torch.pi*f*self.features)
        s = 0.5*torch.sin(2.*torch.pi*f*self.features)
...
#             print(self.features)
            ...
            a1 = 0.4
            a2 = 0.4
            x = a1*torch.randn(self.features.size())
            y = a2*torch.randn(self.features.size())
            self.labels = torch.hstack((x,y))


        elif mode == 'shot': # shot noise, convolute gaussian noise w/ exponential kernel
            tau=20
            windowSize=50
            win=torch.linspace(1,windowSize+1,windowSize)
            k=torch.exp(-win/tau)

            a1 = 0.4
            a2 = 0.4
            # noise=np.random.randn(tf+len(win)-1)
            nx = a1*torch.randn(timesteps+windowSize-1)
            ny = a2*torch.randn(timesteps+windowSize-1)


            # kernel = torch.zeros(windowSize))
            # for i in range(2):
            #     kernel[i,:] = k

            sx = torchaudio.functional.convolve(nx,k,mode='valid')
            sx = torch.reshape(sx,(timesteps,1,1))
            sy = torchaudio.functional.convolve(ny,k,mode='valid')
            sy = torch.reshape(sy,(timesteps,1,1))
            self.labels = torch.hstack((sx,sy))
            # self.labels = torch.reshape()
