...
for pcon in pconvec:
    ...
    if seednum == 0:
        ...
        #stim_low = np.array(label1[:,:,0].cpu())
        #np.save(f'rawData/stim_low.npy',stim_low)
        #stim_high = np.array(label2[:,:,0].cpu())
        #np.save(f'rawData/stim_high.npy',stim_high)
    ...
    np.save(f'rawData/Nh{Nh}/pcon{pcon}/seed{seednum}/readout.npy',readout.cpu().detach().numpy())
