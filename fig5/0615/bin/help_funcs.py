...
def switchProcess2D(tvec,mu,sigma):
    N = len(tvec)
    mu_wait = mu
    sigma_wait = sigma
    tf = tvec[-1]
    dt = tvec[-1] - tvec[-2]
    num_wait_times = int(tf/mu_wait) + 100
    wait_times = np.random.normal(mu_wait,sigma_wait,(num_wait_times,2))
    yvec = np.zeros((N,2))
    y0 = np.random.randint(0,2,2) - 0.5
    yvec[0,:] = y0

    wix = 0 # x index of wait_times
    wiy = 0 # y index
    nx_switch = 0 # most recent switch time for x
    ny_switch = 0 #
    for n in range(N-1):
        yvec[n+1] = yvec[n]
        if n - nx_switch > int(wait_times[wix,0]/dt):
            nx_switch = n
            yvec[n+1,0] = -yvec[n,0]
            wix += 1
        if n - ny_switch > int(wait_times[wiy,1]/dt):
            ny_switch = n
            yvec[n+1,1] = -yvec[n,1]
            wiy += 1
    return yvec
...
#             print(self.features)
            ...
            mu = 100
            sigma = 30
            tvec = np.arange(0,timesteps,1)
#             print(f'tvec = {tvec}')
            yvec = switchProcess2D(tvec,mu,sigma)
            x = torch.Tensor(yvec[:,0])
            x = torch.reshape(x,self.features.size())
            y = torch.Tensor(yvec[:,1])
            y = torch.reshape(y,self.features.size())
