...
def makeRaster(Ns, stim, spikeTimes, spikeIndices, E1i):
    stin, stE1, stB = spikeTimes
    siin, siE1, siB = spikeIndices
    Nin, NE1, NB = Ns
    
    if not os.path.exists('rasters'):
        os.mkdir('rasters')
    fig, ax = plt.subplots(4,figsize=(9,12),gridspec_kw={'height_ratios': [Nin, Nin, NE1, NB]})
...
def getCorrPlots(st,si,stim,N_nrn,tf,T_R,dt,delta_t,E1i,delay,layerName):
...
def getReconstruction(tvecSub,y_test,y_pred_count,y_pred_rank,layerName,E1i):
...
def getStimDist(y_test,y_pred_count,y_pred_rank,layerName,E1i):
