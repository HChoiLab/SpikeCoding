...
f = float(sys.argv[3]) # stim frequency
pcon = 0.3
Nout = 100
Ns = [Nin, Nh, Nout]
#layerNames = ['in', 'h', 'out']
...
num_iter = 50 # train for this many iterations
...
if not os.path.exists('lossPlots'):
    os.mkdir('lossPlots')
if not os.path.exists(f'lossPlots/f{int(f)}'):
    os.mkdir(f'lossPlots/f{int(f)}')
plt.plot(loss_hist,color='black')
plt.ylabel('mse loss',fontsize=18)
plt.xlabel('iteration no.',fontsize=18)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
fname = f'lossPlots/f{int(f)}/loss_v_iter_Nh{Nh}_seed{seednum}.png'
plt.savefig(fname,bbox_inches='tight',dpi=200)
plt.close()

plot2Dfit(s[:tmax,:,:].cpu(),readout.cpu(),seednum,Nh,tag)
...
if not os.path.exists(f'trainedModels/Nh{Nh}/f{int(f)}'):
    os.mkdir(f'trainedModels/Nh{Nh}/f{int(f)}')
fname = f'trainedModels/Nh{Nh}/f{int(f)}/seed{seednum}.pth'
